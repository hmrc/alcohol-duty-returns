...
class AdrReturnsSubmissionSpec extends SpecBase {
  "AdrReturnsSubmission" should {
    "deserialise from json" in new SetUp {
      Json.parse(json).as[AdrReturnsSubmission] shouldBe adrReturnsSubmission
    }
  }
  ...
  class SetUp {
    ...
    val adrReturnsSubmission = exampleReturnsSubmissionRequest
  }
}